...
- (void)updateView {
    ...
    if (appDelegate.session.isOpen) {        
        ...
        [self.textNoteOrLink setText:[NSString stringWithFormat:@"https://graph.facebook.com/me/photos?access_token=%@",
...
- (IBAction)buttonClickHandler:(id)sender {
    ...
    if (appDelegate.session.isOpen) {
        ...
        NSString *graphPath = @"me/photos?fields=tags&limit=1";
        ...
        if (appDelegate.session.state != FBSessionStateCreated) {
            ...
            [FBSession openActiveSessionWithReadPermissions:@[@"user_photos", @"user_likes"]
                                               allowLoginUI:YES
                                          completionHandler:^(FBSession *session, FBSessionState status, NSError *error) {
                                              [self updateView];
                                          }];
